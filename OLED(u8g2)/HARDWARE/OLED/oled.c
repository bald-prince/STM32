...
//  m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE];
    ...
    for(i = 0; i < MAX_MODULESIZE; i++)
    {
        for(j = 0; j < MAX_MODULESIZE; j++)
        {
            ...
            {
            //2倍放大，1个点放大为4个点
                OLED_DrawPoint(2*i, 2*j,mode);
                OLED_DrawPoint(2*i, 2*j+1,mode);        
                OLED_DrawPoint(2*i+1, 2*j,mode);  
                OLED_DrawPoint(2*i+1, 2*j+1,mode);
...
uint8_t u8x8_stm32_gpio_and_delay(U8X8_UNUSED u8x8_t *u8x8,U8X8_UNUSED uint8_t msg, U8X8_UNUSED uint8_t arg_int,U8X8_UNUSED void *arg_ptr)
{
  switch (msg)
  {
		case U8X8_MSG_GPIO_SPI_DATA:
			(arg_int)?SET_BIT(GPIOD->ODR,GPIO_PIN_13):CLEAR_BIT(GPIOD->ODR,GPIO_PIN_13);   //D1
			break;
		case U8X8_MSG_GPIO_SPI_CLOCK:
			(arg_int)?SET_BIT(GPIOD->ODR,GPIO_PIN_12):CLEAR_BIT(GPIOD->ODR,GPIO_PIN_12);
			break;
		...
		case U8X8_MSG_DELAY_MILLI:
			...
			case U8X8_MSG_GPIO_CS:
				(arg_int)?SET_BIT(GPIOB->ODR,GPIO_PIN_12):CLEAR_BIT(GPIOB->ODR,GPIO_PIN_12);
		case U8X8_MSG_GPIO_DC:
					(arg_int)?SET_BIT(GPIOB->ODR,GPIO_PIN_11):CLEAR_BIT(GPIOB->ODR,GPIO_PIN_11);
			break;
		case U8X8_MSG_GPIO_RESET:
					(arg_int)?SET_BIT(GPIOB->ODR,GPIO_PIN_10):CLEAR_BIT(GPIOB->ODR,GPIO_PIN_10);
			break;
  }
  return 1;
}
