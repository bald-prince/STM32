...
typedef struct
{
  ...
  PID_type kd;                  // * (D)erivative Tuning Parameter 微分环节系数
	...
	he controllerDirection;		//DIRECT  0  : PID输出增加 控制量增加
															//REVERSE  1 : PID输出增加，控制量减少  参考空调制冷，输出功率增加，空调制冷增强，越冷
	he pOn;

  int16_t *myInput;              // * Input, Output, and Setpoint 变量的指针
  int16_t *myOutput;             //   通过这三个指针紧密地联系了变量和PID
  int16_t *mySetpoint;           //   而不需要编程者持续关注变量的值
                                  //   使用指针就能取到了
	...
	he inAuto, pOnE;							//inAuto:用于设置PID工作模式		AUTOMATIC(1):自动模式，由函数自动实现PID计算
...
*/
void new_PID(PID_Class *mpid,int16_t* Input,int16_t* Output,int16_t* Setpoint,double Kp,double Ki,double Kd,he POn, he ControllerDirection);//初始化一个新的PID类	
