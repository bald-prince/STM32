...
/*	Show_Str(15,95,200,12,"连接状态:",12,0); 		//连接状态
	...
	printf((u8*)ATK_ESP8266_WORKMODE_TBL[netpro&0X03]);
	printf("\n");
	printf((u8*)ATK_ESP8266_WORKMODE_TBL[netpro>>4]);
	printf("\n");
	USART3_RX_STA=0;
	while(1)
	{
...
void esp_8266_send(u8 send_num)
{
	u8 *p;
	p=mymalloc(SRAMIN,100);
	key_buf = 0;
