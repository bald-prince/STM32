...
{	
	...
	HAL_UART_Receive_IT(&UART2_Handler,(u8 *)a2RxBuffer,1);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    rec1=*(--(huart->pRxBuffPtr));
		//printf((&rec1));
	  retransmit[counter] = rec1;
		counter ++;
		if(counter == 8)
		{
			retransmit[8]= '/0';
			for(counter=0;counter<8;counter++)
			printf("%c",retransmit[counter]);
		  counter = 0;
		}
	}
	else;
	if(huart->Instance==USART2)//如果是串口2
	{
		...
		if((USART2_RX_STA&0x8000)==0)//接收未完成
		{
			...
			{
				if(a2RxBuffer[0]!=0x0a)USART2_RX_STA=0;//接收错误,重新开始
				else USART2_RX_STA|=0x8000;	//接收完成了 
			}
			else //还没收到0X0D
			{	
				if(a2RxBuffer[0]==0x0d)USART2_RX_STA|=0x4000;
				else
				{
					USART2_RX_BUF[USART2_RX_STA&0X3FFF]=a2RxBuffer[0] ;
					USART2_RX_STA++;
...
//#endif
	...
	timeout=0;
	while(HAL_UART_Receive_IT(&UART2_Handler, (u8 *)a2RxBuffer, RXBUFFERSIZE) != HAL_OK)//一次处理完成之后，重新开启中断并设置RxXferCount为1
	{
	 timeout++; //超时处理
	 if(timeout>HAL_MAX_DELAY) break;	
	}
